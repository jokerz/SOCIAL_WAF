...
# @desc		
#			
# @package	MyClass::JKZDB::CmsMember
# @access	public
# @author	Iwahase Ryo AUTO CREATE BY ./createClassDB
# @create	Wed Jan 13 19:08:08 2010
# @version	1.30
...
sub new {
	my ($class, $dbh) = @_;
	my $table = 'dMOEPETIT.tCmsMemberM';
...
$self->{columnslist}->{uid}->[$i] = $aryref->[$i]->[0];
$self->{columnslist}->{login_name}->[$i] = $aryref->[$i]->[1];
$self->{columnslist}->{login_password}->[$i] = $aryref->[$i]->[2];
$self->{columnslist}->{status_flag}->[$i] = $aryref->[$i]->[3];
$self->{columnslist}->{name}->[$i] = $aryref->[$i]->[4];
$self->{columnslist}->{level}->[$i] = $aryref->[$i]->[5];
$self->{columnslist}->{description}->[$i] = $aryref->[$i]->[6];
...
#************************ AUTO GENERATED BEGIN ************************
			(
$self->{columns}->{uid},
$self->{columns}->{login_name},
$self->{columns}->{login_password},
...
sub executeUpdate {
	...
	$self->{columns}->{uid} = $param->{uid};

	## ここでPrimaryKeyが設定されている場合はUpdate
	## 設定がない場合はInsert
	if ($self->{columns}->{uid} < 0) {
...
## Modified 2009/09/29 BEGIN
		...
		#push( @{ $sqlref->[0] }, "uid" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{uid} ) if $param->{uid} != "";
		push( @{ $sqlref->[0] }, "login_name" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{login_name} ) if $param->{login_name} ne "";
		push( @{ $sqlref->[0] }, "login_password" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{login_password} ) if $param->{login_password} ne "";
		...
		push( @{ $sqlref->[0] }, "level" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{level} ) if $param->{level} != "";
		...
		$sqlMoji = sprintf("UPDATE $self->{table} SET %s =?  WHERE uid= '$self->{columns}->{uid}';", join('=?,', @{ $sqlref->[0] }));
		#$rv = $self->executeQuery($sqlMoji, $sqlref->[2]);
