...
# @desc        
#            
# @package    MyClass::JKZDB::GsaSubCategory
# @access    public
# @author    Iwahase Ryo AUTO CREATE BY ./createClassDB
# @create    Thu Jan 21 12:18:09 2010
# @version    1.30
...
# @version    1.30
#******************************************************
package MyClass::JKZDB::SubCategory;
...
sub new {
    my ($class, $dbh) = @_;
    my $table = 'dMOEPETIT.tSubCategoryM';
...
$self->{columnslist}->{subcategory_id}->[$i] = $aryref->[$i]->[0];
$self->{columnslist}->{categorym_id}->[$i] = $aryref->[$i]->[1];
$self->{columnslist}->{status_flag}->[$i] = $aryref->[$i]->[2];
$self->{columnslist}->{subcategory_name}->[$i] = $aryref->[$i]->[3];
$self->{columnslist}->{category_name}->[$i] = $aryref->[$i]->[4];
...
sub executeUpdate {
    ...
    $self->{columns}->{subcategory_id} = $param->{subcategory_id};

    ## ここでPrimaryKeyが設定されている場合はUpdate
    ## 設定がない場合はInsert
    if ($self->{columns}->{subcategory_id} < 0) {
...
## Modified 2009/09/29 BEGIN
        ...
        #push( @{ $sqlref->[0] }, "subcategory_id" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{subcategory_id} ) if $param->{subcategory_id} != "";
        ...
        $sqlMoji = sprintf("UPDATE $self->{table} SET %s =?  WHERE subcategory_id= '$self->{columns}->{subcategory_id}';", join('=?,', @{ $sqlref->[0] }));
        #$rv = $self->executeQuery($sqlMoji, $sqlref->[2]);
