...
# @desc      
#            
# @package    MyClass::JKZDB::DegreeImage
# @access    public
# @author    Iwahase Ryo AUTO CREATE BY createClassDB
# @create    Wed Jun  8 13:26:47 2011
# @version    1.30
...
sub new {
    my ($class, $dbh) = @_;
    my $table = 'dMOEPETIT.tDegreeImageM';
...
$self->{columnslist}->{degreem_id}->[$i] = $aryref->[$i]->[0];
$self->{columnslist}->{image}->[$i] = $aryref->[$i]->[1];
$self->{columnslist}->{resized_image}->[$i] = $aryref->[$i]->[2];
$self->{columnslist}->{mime_type}->[$i] = $aryref->[$i]->[3];
#************************ AUTO  GENERATED  END ************************
        }
    }
...
=pod
        ...
        $sqlMoji = sprintf("UPDATE $self->{table} SET %s =?  WHERE degreem_id= '$self->{columns}->{degreem_id}';", join('=?,', @{ $sqlref->[0] }));
