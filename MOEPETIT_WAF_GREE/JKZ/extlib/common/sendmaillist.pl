...
#############################
my @LOOKUP = (
	{'setting' => 'HP_management.tAdmailsetting', 'body' => 'HP_management.tAdcontents',
		'manage' => 'HP_management.tAdmailmanage'.$id,	'id_field' => 'admailno'},
...
$sth=$db->prepare("select distinct m.owid, m.password, m.cryptpassword, m.carrier, m.mobilemailaddress
 from HP_general.tMemberM m, $look_ref->{'manage'} t where m.owid = t.id_no");
