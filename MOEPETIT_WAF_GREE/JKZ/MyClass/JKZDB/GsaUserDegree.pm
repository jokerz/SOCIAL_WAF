...
# @desc      
#            
# @package    MyClass::JKZDB::GsaUserDegree
# @access    public
# @author    Iwahase Ryo AUTO CREATE BY createClassDB
# @create    Wed Jun  8 13:27:11 2011
# @version    1.30
...
sub new {
    my ($class, $dbh) = @_;
    my $table = 'dMOEPETIT.tGsaUserDegreeF';
...
$self->{columnslist}->{degreem_id}->[$i] = $aryref->[$i]->[1];
$self->{columnslist}->{degree_name}->[$i] = $aryref->[$i]->[2];
...
sub executeUpdate {
    ...
    $self->{columns}->{degreem_id} = $param->{degreem_id};

    ## ここでPrimaryKeyが設定されている場合はUpdate
    ## 設定がない場合はInsert
    #if ($self->{columns}->{degreem_id} < 0) {}
    if ($flag < 0) {
        ...
        #push( @{ $sqlref->[0] }, "degreem_id" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{degreem_id} ) if $param->{degreem_id} != "";
        ...
        $sqlMoji = sprintf("UPDATE $self->{table} SET %s =?  WHERE gree_user_id='$self->{columns}->{gree_user_id}' AND degreem_id= '$self->{columns}->{degreem_id}';", join('=?,', @{ $sqlref->[0] }));
