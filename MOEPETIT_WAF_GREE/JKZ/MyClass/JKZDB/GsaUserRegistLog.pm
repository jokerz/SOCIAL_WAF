...
# @desc      
#            
# @package    MyClass::JKZDB::GsaUserRegistLog
# @access    public
# @author    Iwahase Ryo AUTO CREATE BY ./createClassDB
# @create    Fri Feb  4 17:45:07 2011
# @version    1.30
...
sub new {
    my ($class, $dbh) = @_;
    my $table = 'dMOEPETIT.tGsaUserRegistLogF';
...
$self->{columnslist}->{date_of_transaction}->[$i] = $aryref->[$i]->[5];
#************************ AUTO  GENERATED  END ************************
        }
    }
...
sub executeUpdate {
    ...
    if ($self->{columns}->{id} < 0) {
        ...
        push( @{ $sqlref->[0] }, "date_of_transaction" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{date_of_transaction} ) if $param->{date_of_transaction} ne "";
