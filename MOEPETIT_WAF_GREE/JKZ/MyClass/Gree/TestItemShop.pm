...
# @desc      
# @desc      ItemShopのクラス
# @package   MyClass::Gree::TestItemShop
# @access    public
# @author    Iwahase Ryo
# @create    2011/04/04
# @update    
...
use constant {
    ...
    REQUEST_METHOD        =>  'POST',
    REQUEST_URI           =>  'https://os.gree.jp/api/rest/payment/@me/@self/@app',
};
#=cut
...
sub confirm_order {
    ...
    $encoded_item_name        = MyClass::WebUtil::convertByNKF('-w', $q->unescape($encoded_item_name));
    $encoded_item_description = MyClass::WebUtil::convertByNKF('-w', $q->unescape($encoded_item_description));
    $encoded_item_detail      = MyClass::WebUtil::convertByNKF('-w', $q->unescape($encoded_item_detail));

    my $imageUrl  = sprintf("%s?ii=%s", $self->ITEMIMAGE_SCRIPTDATABASE_URL, $obj->{ITEM_ID});
    my $finishUrl = sprintf("%s&a=finish&o=order", $self->ITEMSHOP_URL);

# SB用
=pod
    my $data = {
        callbackUrl     => 'http://st.moepetit.jp/event/paymentapp.mpl',
        finishPageUrl   => $finishUrl,
...
#=pod
    my $data = {
        callbackUrl     => 'http://www.moepetit.jp/event/paymentapp.mpl',
        finishPageUrl   => $finishUrl,
        message         => $encoded_item_detail,
        paymentItems    => [
            {
 ...
 ## Modified 2011/04/26 文字化け対応
    ...
    my $res = $ua->request($req) or die 'Failed to request';
        ...
        #*********************************
            ...
            eval {
                $myItemOrder->executeUpdate(
                    {
                        paymentId           => $obj->{paymentId},
                        status              => $obj->{status},
                        orderTime           => $obj->{orderedTime},
                        gree_user_id        => $self->opensocial_viewer_id,
                        itemm_id            => $item_id          ,
                        item_type           => $item_type,
                        item_categorym_id   => $item_categorym_id,
...
#=pod
    ...
    my $itembox_name_space		= sprintf("%s_gsa_user_itembox", $namespace);
    my $stopwatch_name_space	= sprintf("%s_gsa_user_stopwatch", $namespace);
    
    $self->memcached->delete("$itembox_name_space:$gree_user_id:offest:$offset");
    $self->memcached->delete("$stopwatch_name_space:$gree_user_id:offest:$offset");
...
sub view_itemshop {
    ...
    ## Modified 2011/05/19 額縁は一時的になし BEGIN
    if(7 == $item_category_id) {
    ...
    ## Modified 2011/05/19 額縁は一時的になし END
