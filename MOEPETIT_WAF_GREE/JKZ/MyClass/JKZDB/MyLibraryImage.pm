...
# @desc      
#            
# @package    MyClass::JKZDB::MyLibraryImage
# @access    public
# @author    Iwahase Ryo AUTO CREATE BY createClassDB
# @create    Mon May  2 19:41:44 2011
# @version    1.30
...
sub new {
    my ($class, $dbh) = @_;
    my $table = 'dMOEPETIT.tMyLibraryImageM';
...
$self->{columnslist}->{my_getimage_id}->[$i] = $aryref->[$i]->[1];
$self->{columnslist}->{my_library_binding_image_flag}->[$i] = $aryref->[$i]->[2];
$self->{columnslist}->{image_order}->[$i] = $aryref->[$i]->[3];
...
$self->{columns}->{my_getimage_id},
$self->{columns}->{my_library_binding_image_flag},
$self->{columns}->{image_order},
$self->{columns}->{image},
...
sub executeUpdate {
    ...
    $self->{columns}->{my_getimage_id} = $param->{my_getimage_id};
    $self->{columns}->{my_library_id}  = $param->{my_library_id};
    ## ここでPrimaryKeyが設定されている場合はUpdate
    ## 設定がない場合はInsert
    #if ($self->{columns}->{my_getimage_id} < 0) {
        ...
        push( @{ $sqlref->[0] }, "my_getimage_id" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{my_getimage_id} ) if $param->{my_getimage_id} != "";
        push( @{ $sqlref->[0] }, "my_library_binding_image_flag" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{my_library_binding_image_flag} ) if $param->{my_library_binding_image_flag} != "";
        push( @{ $sqlref->[0] }, "image_order" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{image_order} ) if $param->{image_order} != "";
...
=pod
        ...
        $sqlMoji = sprintf("UPDATE $self->{table} SET %s =?  WHERE my_getimage_id= '$self->{columns}->{my_getimage_id}';", join('=?,', @{ $sqlref->[0] }));
